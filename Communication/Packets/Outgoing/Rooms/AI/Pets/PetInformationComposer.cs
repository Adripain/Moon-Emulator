...
namespace Moon.Communication.Packets.Outgoing.Rooms.AI.Pets
{
    class PetInformationComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.PetInformationMessageComposer)
        {
            ...
            if (!MoonEnvironment.GetGame().GetRoomManager().TryGetRoom(Pet.RoomId, out Room))
                return;
            ...
            : base(ServerPacketHeader.PetInformationMessageComposer)
        {
            ...
            base.WriteInteger(Habbo.Id);
            base.WriteInteger(Convert.ToInt32(Math.Floor((MoonEnvironment.GetUnixTimestamp() - Habbo.AccountCreated) / 86400)));//How?
            base.WriteString(Habbo.Username);
            base.WriteInteger(1);//3 on hab
            base.WriteBoolean(false);
            base.WriteBoolean(false);
