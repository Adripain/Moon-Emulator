...
namespace Moon.Communication.Packets.Outgoing.Rooms.Engine
{
    class UsersComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.UsersMessageComposer)
        {
            ...
            //{
            //    base.WriteInteger(0);//0(cero) avatar
            //} else
            //{
        ...
        private void WriteUser(RoomUser User)
        {
            if (!User.IsPet && !User.IsBot)
            {
                ...
                if (Habbo != null)
                {
                    if (Habbo.GetStats() != null)
                    {
                        ...
                        {
                            if (!MoonEnvironment.GetGame().GetGroupManager().TryGetGroup(Habbo.GetStats().FavouriteGroupId, out Group))
                                Group = null;
                        }
                    }
                }
            ...
            else if (User.IsBot)
            {
                ...
                base.WriteString(MoonEnvironment.GetUsernameById(User.BotData.ownerID)); // Owner name
