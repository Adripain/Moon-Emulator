...
namespace Moon.Communication.Packets.Incoming.Groups
{
    class GetForumsListDataEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            {
                case 2:
                    var Forums = MoonEnvironment.GetGame().GetGroupForumManager().GetForumsByUserId(Session.GetHabbo().Id);
                ...
                case 0:
                    ...
                    using (var adap = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
                    {
                        ...
                        adap.AddParameter("now", (int)MoonEnvironment.GetUnixTimestamp());
                    ...
                    {
                        GroupForum forum;
                        if (MoonEnvironment.GetGame().GetGroupForumManager().TryGetForum(Convert.ToInt32(Row["id"]), out forum))
                            forums.Add(forum);
                    }
                    break;
