...
namespace Moon.HabboHotel.Rooms.AI.Types
{
    class RouleteBotDucket : BotAI
    {
        ...
        public RouleteBotDucket(int VirtualId)
        {
                ...
                foreach (KeyValuePair<int, CasinoDataLocura> pair in Data1)
                {
                    ...
                    if (Data2.bet == num)
                    {
                        ...
                        if (client is GameClient)
                        {
                            ...
                            client.GetHabbo().Duckets += d;
                            client.SendMessage(new HabboActivityPointNotificationComposer(client.GetHabbo().Duckets, d));
