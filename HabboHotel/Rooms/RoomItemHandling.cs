...
namespace Moon.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
                ...
                if (_movedItems.Count > 0)
                {
                    foreach (Item Item in _movedItems.Values.ToList())
                    {
                        using (IQueryAdapter dbClient = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
                        {
                            ...
                            {
                                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RoomDecoFloor", 1);
                            }
                            if (Item.GetBaseItem().ItemName.Contains("floor_single"))
                            {
                                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RoomDecoWallpaper", 1);
                            }
                            if (Item.GetBaseItem().ItemName.Contains("landscape_single"))
                            {
                                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RoomDecoLandscape", 1);
                            }
                            // if (Item.GetBaseItem().ItemName == "es_skating_ice")
                            // {
                            //     MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_TagA", 1);
                            // }
                            if (Item.GetBaseItem().ItemName.Contains("val11_floor"))
                            {
                                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RbTagA", 1);
                            }
                            /* if (Item.GetBaseItem().ItemName == "easter11_grasspatc")
                             {
                                 MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RbBunnyTa", 1);
                             }
                             if (Item.GetBaseItem().ItemName == "hole" || Item.GetBaseItem().ItemName == "hole2")
                             {
                                 MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_RoomDecoHoleFurniCount", 1);
                             }
                             if (Item.GetBaseItem().ItemName == "snowb_slope")
                             {
                                 MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(Client, "ACH_snowBoardBuild", 1);
                             }  */
        ...
        public bool SetFloorItem_ball(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false, bool ball = false)
        {
            ...
            if (newItem)
            {
                if (_floorItems.ContainsKey(Item.Id))
                {
                    if (Session != null && ball == false)
                        Session.SendNotification(MoonEnvironment.GetGame().GetLanguageLocale().TryGetValue("room_item_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }
