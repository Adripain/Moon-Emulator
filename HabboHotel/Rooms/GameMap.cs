...
namespace Moon.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            mStaticModel = MoonEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            if (mStaticModel == null)
            {
                MoonEnvironment.GetGame().GetRoomManager().LoadModel(room.ModelName);
                mStaticModel = MoonEnvironment.GetGame().GetRoomManager().GetModel(room.ModelName);
            }
        ...
        public Point getRandomWalkableSquare()
        {
            ...
            int RandomNumber = MoonEnvironment.GetRandomNumber(0, walkableSquares.Count);
            int i = 0;
            ...
            if (mCoordinatedItems.ContainsKey(coord))
            {
                ...
                foreach (Item item in itemsFromSquare)
                {
                    if (item.GetBaseItem().InteractionType == InteractionType.RENTABLE_SPACE)
                        ...
                        if (MoonEnvironment.GetGame().GetRentableSpaceManager().GetRentableSpaceItem(item.Id, out _rentableSpace))
                        {
                            ...
                            if (_rentableSpace.OwnerId == Session.GetHabbo().Id && MoonEnvironment.GetUnixTimestamp() < _rentableSpace.ExpireStamp)
                            {
