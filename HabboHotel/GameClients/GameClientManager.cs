...
namespace Moon.HabboHotel.GameClients
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger("Moon.HabboHotel.GameClients.GameClientManager");
        ...
        private ConcurrentDictionary<Guid, Session> _sessionRegister;
        ...
        public GameClientManager()
        {
            ...
            this._sessionRegister = new ConcurrentDictionary<Guid, Session>();
        ...
        /*public void sessionHandleMessage(IWebSocketConnection socket, byte[] message)
        {
            ...
            if (this._sessionRegister.TryGetValue(socket.ConnectionInfo.Id, out Session session))
            {
        ...
        {
            if (!this._sessionRegister.TryRemove(socket.ConnectionInfo.Id, out Session session))
                socket.Close();
        }
        ...
        public void registerSession(IWebSocketConnection socket)
        {
            ...
            if (!this._sessionRegister.TryAdd(socket.ConnectionInfo.Id, session))
                socket.Close();
        }*/
        ...
        public void MsgAlert(ServerPacket Message, int Exclude = 0)
        {
            foreach (GameClient client in this.GetClients.ToList())
            {
                if (client == null || client.GetHabbo() == null)
