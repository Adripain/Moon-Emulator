...
namespace Moon.HabboHotel.Achievements
{
    ...
    public class TalentManager
    {
        ...
        public void CompleteUserTalent(GameClient session, Talent talent)
        {
            ...
            {
                ItemData item = null;
                if (!MoonEnvironment.GetGame().GetItemManager().GetItem(talent.PrizeBaseItem, out item))
                    return;
            ...
            using (IQueryAdapter queryReactor = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
            {
            ...
            if (talent.Type == "citizenship")
            {
                ...
                {
                    case 3:
                        MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(session, "ACH_Citizenship", 1);
                        break;
                    case 4:
