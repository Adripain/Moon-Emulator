...
namespace Moon.HabboHotel.Groups
{
    public class Group
    {
        ...
        public void CreateGroupChat(Group group)
        {
            ...
            group.HasChat = true;
            List<GameClient> GroupMembers = (from Client in MoonEnvironment.GetGame().GetClientManager().GetClients.ToList() where Client != null && Client.GetHabbo() != null && this.IsMember(Client.GetHabbo().Id) select Client).ToList();
        ...
        //    foreach (var client in MoonEnvironment.GetGame().GetClientManager().GetClientsById(GetAllMembers))
        //    {
