    ...
    public class HabboMessenger
    {
        ...
        public void ProcessOfflineMessages()
        {
            ...
            using (IQueryAdapter dbClient = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (GetMessages != null)
                {
                    GameClient Client = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
                    if (Client == null)
                        return;
        ...
        public void Destroy()
        {
            IEnumerable<GameClient> onlineUsers = MoonEnvironment.GetGame().GetClientManager().GetClientsById(_friends.Keys);
        ...
        public void CreateFriendship(int friendID)
        {
            ...
            GameClient User = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (User != null)
                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User, "ACH_FriendListSize", 1);

            GameClient thisUser = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(_userId);
            if (thisUser != null)
                MoonEnvironment.GetGame().GetAchievementManager().ProgressAchievement(thisUser, "ACH_FriendListSize", 1);
        }
        ...
        {
            GameClient friend = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(friendID);
            ...
            if (friend == null || friend.GetHabbo() == null)
            {
                ...
                newFriend = new MessengerBuddy(friendID, Convert.ToString(dRow["username"]), Convert.ToString(dRow["look"]), Convert.ToString(dRow["motto"]), Convert.ToInt32(dRow["last_online"]),
                    MoonEnvironment.EnumToBool(dRow["hide_online"].ToString()), MoonEnvironment.EnumToBool(dRow["hide_inroom"].ToString()));
            }
            else
            {
        ...
        public bool RequestBuddy(string UserQuery)
        {
            ...
            GameClient client = MoonEnvironment.GetGame().GetClientManager().GetClientByUsername(UserQuery);
            if (client == null)
            {
                DataRow Row = null;
                using (IQueryAdapter dbClient = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
                ...
                hasFQDisabled = MoonEnvironment.EnumToBool(Row["block_newfriends"].ToString());
            }
            else
            {
            ...
            MoonEnvironment.GetGame().GetQuestManager().ProgressUserQuest(GetClient(), QuestType.ADD_FRIENDS);

            GameClient ToUser = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (ToUser == null || ToUser.GetHabbo() == null)
                return true;

            MessengerRequest Request = new MessengerRequest(ToId, _userId, MoonEnvironment.GetGame().GetClientManager().GetNameById(_userId));

            ToUser.GetHabbo().GetMessenger().OnNewRequest(_userId);

            UserCache ThisUser = MoonEnvironment.GetGame().GetCacheManager().GenerateUser(_userId);
        ...
        public void OnNewRequest(int friendID)
        {
            if (!_requests.ContainsKey(friendID))
                _requests.Add(friendID, new MessengerRequest(_userId, friendID, MoonEnvironment.GetGame().GetClientManager().GetNameById(friendID)));
        }
        ...
        public void SendInstantMessage(int ToId, string Message)
        {
            ...
            #region Custom Chats
            var Group = MoonEnvironment.GetGame().GetGroupManager().GetGroupsForUser(GetClient().GetHabbo().Id).Where(c => c.HasChat).ToList();
            foreach (var gp in Group)
            {
                if (ToId == int.MinValue + gp.Id) // int.MaxValue
                {
                    //MoonEnvironment.GetGame().GetClientManager().SendMessaget(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                    MoonEnvironment.GetGame().GetClientManager().GroupChatAlert(new FuckingConsoleMessageComposer(int.MinValue + gp.Id, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), gp, GetClient().GetHabbo().Id);
                    return;
                }
            ...
            {
                MoonEnvironment.GetGame().GetClientManager().StaffAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            else if (GetClient().GetHabbo()._guidelevel >= 1 && ToId == (int.MinValue + 1))
            {
                MoonEnvironment.GetGame().GetClientManager().GuideAlert(new FuckingConsoleMessageComposer(ToId, Message, GetClient().GetHabbo().Username + "/" + GetClient().GetHabbo().Look + "/" + GetClient().GetHabbo().Id), GetClient().GetHabbo().Id);
                return;
            }
            #endregion
            ...
            {
                GetClient().GetHabbo().MessengerSpamTime = MoonEnvironment.GetUnixTimestamp() + 60;
            ...
            else if (GetClient().GetHabbo().MessengerSpamTime > MoonEnvironment.GetUnixTimestamp())
            {
                double Time = GetClient().GetHabbo().MessengerSpamTime - MoonEnvironment.GetUnixTimestamp();
            ...
            GameClient Client = MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(ToId);
            if (Client == null || Client.GetHabbo() == null || Client.GetHabbo().GetMessenger() == null)
            {
                using (IQueryAdapter dbClient = MoonEnvironment.GetDatabaseManager().GetQueryReactor())
                {
        ...
        {
            IEnumerable<GameClient> MyFriends = MoonEnvironment.GetGame().GetClientManager().GetClientsById(this._friends.Keys);
        ...
        {
            return MoonEnvironment.GetGame().GetClientManager().GetClientByUserID(this._userId);
        }
